...
func TestApplyMergeStrategyBatch(t *testing.T) {
	...
	// Verification reads for the two bool children return empty (non-fatal).
	mock := &gh.MockRunner{
		Responses: map[string][]byte{
	...
	// 1 companion fetch (squash_merge_commit_message) + 1 batched PATCH +
	// 2 post-apply verification reads (allow_squash_merge, delete_branch_on_merge).
	// squash_merge_commit_title is a string field and is skipped by verification.
	// Verification reads return empty output (non-fatal), so Err stays nil.
	if len(mock.Called) != 4 {
		t.Fatalf("expected 4 gh calls (1 companion fetch + 1 PATCH + 2 verify reads), got %d", len(mock.Called))
	}
